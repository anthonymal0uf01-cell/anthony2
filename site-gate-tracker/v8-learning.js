...
async function storeActiveFrame(t,caseId,seq){
 const blob=await captureTrackBlob(t);if(!blob)return false;const q=await blobQuality(blob);if(!q)return false;const hash=await hashFrame(),h=hardScore(q,t),existing=(await all()).filter(x=>x.trackId===t.id);
...
async function maybeCapture(){
 ...
 const tracks=coreTracks();if(!tracks.size)return;const ranked=[];
 for(const t of tracks.values()){const blob=await captureTrackBlob(t);if(!blob)continue;const q=await blobQuality(blob);if(!q)continue;const h=hardScore(q,t);if(h.score<.24&&t.status==='CONFIRMED')continue;ranked.push({t,h:h.score})}
