...
function parseIdentityRows(){
 const rows=[...document.querySelectorAll('#identities .idrow')],out=new Map();for(const r of rows){const c=[...r.children].map(x=>x.textContent.trim());if(c.length<4)continue;const id=(c[0]||'').replace(/^#/,'');const type=c[1]||'';let plateCell=c[2]||'';const status=c[3]||'';plateCell=plateCell.replace(/^\?/,'');const parts=plateCell.split('/').map(normalizePlate).filter(Boolean);if(!id)continue;out.set(id,{id,type,plate:parts[0]||'',plate2:parts[1]||'',status})}activeTracks=out;return out;
}
...
async function maybeCapture(){
 ...
 for(const t of tracks.values()){
   ...
   await put({id,trackId:t.id,type:t.type,status:t.status,candidate:t.plate||'',candidate2:t.plate2||'',label:t.status==='CONFIRMED'?t.plate:'',label2:t.status==='CONFIRMED'?t.plate2:'',quality:q,hardScore:hard,hash,createdAt:Date.now(),blob});
