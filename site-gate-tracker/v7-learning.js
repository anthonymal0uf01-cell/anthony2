...
function parseIdentityRows(){
 const rows=[...document.querySelectorAll('#identities .idrow')],out=new Map();for(const r of rows){const c=[...r.children].map(x=>x.textContent.trim());if(c.length<4)continue;const id=(c[0]||'').replace(/^#/,'');const type=c[1]||'';let plateCell=c[2]||'';const status=c[3]||'';plateCell=plateCell.replace(/^\?/,'');const parts=plateCell.split('/').map(normalizePlate).filter(Boolean);if(!id)continue;if(type==='Truck & Dog'&&parts[0]&&!parts[1]&&status==='CONFIRMED')status='PARTIAL';out.set(id,{id,type,plate:parts[0]||'',plate2:parts[1]||'',status})}activeTracks=out;return out;
}
